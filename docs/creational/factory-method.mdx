...
class ShippingService(ABC):
    ...
    def process_order(self, item: str, weight: float) -> None:
        ...
        print(f"📦 {shipping.package(item)}")
        print(f"💰 Cost: ${shipping.calculate_cost(weight):.2f}")
        print(f"🚚 {shipping.deliver()}")
...
function FactoryMethodDemo() {
  ...
  const getShippingMethod = (type) => {
    switch (type) {
      case "standard":
        return createStandardShipping();
      case "express":
        return createExpressShipping();
      case "premium":
        return createPremiumShipping();
      default:
        return createStandardShipping();
    }
  };
  ...
  return (
    <div style={{ fontFamily: "sans-serif" }}>
      ...
      >
        <strong>💡 Factory Method in Action:</strong> Each shipping type is
        created by a different factory function. The{" "}
        <code>getShippingMethod()</code> decides which factory to use, keeping
        the order processing code decoupled from specific shipping
        implementations.
      </div>
    </div>
  );
