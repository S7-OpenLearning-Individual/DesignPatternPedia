...
hide_title: true
description: >
  Just like God, there is only one.
---
...
<Applicability items={[
  {
    situation: "Use the Singleton pattern when a class should have just a single instance",
    context: "For example, a single database object shared by different parts of the program."
  },
  {
    situation: "Use the Singleton pattern when you need stricter control over global variables",
    context: "Unlike global variables, the Singleton pattern guarantees that there's just one instance of a class. Nothing, except for the Singleton class itself, can replace the cached instance."
  }
]} />
...
function SingletonDemo() {
  ...
  const handleCreate = () => {
    ...
    setInstance(obj);
    setLog(l => [...l, `Create Instance → id: ${obj.id}`]);
  };
  const handleReset = () => {
    resetInstance();
    setInstance(null);
...
- [Factory Method](/docs/creational/factory-method.mdx), [Abstract Factory](/docs/creational/abstract-factory.mdx), or [Builder](/docs/creational/builder.mdx) may rely on Singleton to ensure a single configuration or product source is used globally.
- [Facade](/docs/structural/facade.mdx) objects can themselves be singletons that encapsulate a subsystem behind a single interface.
- Unlike [Flyweight](/docs/structural/flyweight.mdx), which manages many shared objects, Singleton restricts creation to precisely one object.
