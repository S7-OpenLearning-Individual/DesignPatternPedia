...
hide_title: true
description: >
  Loop through collections without knowing their secrets.
---
...
function IteratorDemo() {
  ...
  const playNext = () => {
    if (!iteratorRef.current) {
      setLog((l) => [...l, "⚠️ Initialize iterator first"]);
      return;
    }
    ...
    if (iteratorRef.current.hasNext()) {
      ...
      setCurrentSong(song);
      setLog((l) => [...l, `▶️ Playing: ${song}`]);
    } else {
      setCurrentSong(null);
      setLog((l) => [...l, "✓ End of playlist"]);
    }
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.outputBox}>
        {currentSong ? `♪ ${currentSong}` : "No song playing..."}
      </div>
